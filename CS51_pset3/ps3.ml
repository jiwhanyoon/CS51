...
let negate (b : bignum) : bignum =
  if b.coeffs != [] 
  then {neg = not b.neg ; coeffs = b.coeffs} 
  else b
  ;;
...
let less (b1 : bignum) (b2 : bignum) : bool =
  ...
  | false, true -> false
  | _,_ ->
    let a,b = List.length b1.coeffs, List.length b2.coeffs in
    if a = b
    then
      (if max b1.coeffs b2.coeffs = b1.coeffs then b1.neg else not b1.neg)
    else
      (if a < b then not b1.neg else b1.neg)
    ;; 
...
let fromInt (n: int) : bignum =
  let rec makelist (x:int) : int list =
    if x = 0 then [] 
    else 
    makelist ((x - (x mod base))/base) @ [(x mod base)] 
  in 
  {neg = n < 0 ; coeffs = makelist (abs(n))}
;;


let toInt (b : bignum) : int option =
  let x, y = fromInt max_int, fromInt min_int
in
  if greater b x || less b y
  then None
else
  let rec makenum (num:int list) : int =
    match num with
    | [] -> 0
    | h::t -> h + base * (makenum t)
    in
      if b.neg = false 
      then Some (makenum (List.rev (b.coeffs)))
      else Some (- makenum (List.rev (b.coeffs)))
;;
...
let plus (b1: bignum) (b2: bignum) : bignum =
  if b1.neg = false && b2.neg = true 
    then 
      (if b1.coeffs = [] then b2
      else if b2.coeffs = [] then b1
      else if greater b1 (negate b2) then plus_pos b1 b2
      else if equal b1 (negate b2) then (fromInt 0)
      else negate (plus_pos (negate b1) (negate b2))
      )


  else if b1.neg = true && b2.neg = false
    then
      (if b1.coeffs = [] then b2
      else if b2.coeffs = [] then b1
  ...
  else if b1.neg = false && b2.neg = false 
    then plus_pos b1 b2


  else if b1.neg = true && b2.neg = true 
...
(* add two bignums, just play with the coeffs field of the bignum *)
let plusintlists (alst:int list) (blst:int list) : int list =
  let x = plus {neg = false; coeffs = alst} {neg = false; coeffs = blst} 
  in
    x.coeffs
;;

(* times an int with an int list and return the result list *)
let rec timesInt (b: int list) (n: int) (c:int) : int list =
  match b with
  | [] -> if c <> 0 then [c] else []
  | h::t -> 
    let a = h * n + c in 
    let modded = a mod base in
    let carry = a / base in
    modded::timesInt t n carry
;;

(* take in int list as arguement and performing reverse on input and output *)
let makelist (lst: int list) (n: int): int list =
  List.rev (timesInt (List.rev lst) n 0) 
;;

(* times two bignums using the previous implementation of timesInt *)
let rec timesintlists (a:int list) (b:int list): int list=
  match b with
   | [] -> []
   | h::t -> 
    match t with
      | [] -> makelist a h 
      | _ -> plusintlists (makelist a h) (makelist (timesintlists a t) base)
;;

(* the actual times function, also handling result negative sign and times zero *)
let times (b1: bignum) (b2: bignum) : bignum =
  if (b2.coeffs = []) || (b1.coeffs = []) 
  then {neg = false; coeffs = []} 
  else
  {neg = (b1.neg != b2.neg); coeffs = (timesintlists b1.coeffs (List.rev b2.coeffs))}
;;
