...
let super_sum (nlists:int list list) : int =
    let sum (innerlists:int list) : int =
        List.fold_right (fun x y -> x+ y) innerlists 0 
    in
...
let filter_range (nums:int list) (range:int * int) : int list =
     List.filter (fun z -> let x,y = range in z <= y && x <= z) nums
;;
...
let minutes_spent_on_part_1 () : int = 100;;
