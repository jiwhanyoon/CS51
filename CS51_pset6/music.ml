...
let rec list_to_stream (lst : obj list) : event stream =
  let rec list_to_stream_rec nlst =
    ...
    | [] -> list_to_stream lst
    | h ::  t -> match h with
        (* if it's a rest, it has no duration or volume 
        and the pitch is set at pitch , 0 octave*)
        | Rest d -> fun () -> Cons(Tone(0.0, (C,0), 0), 
            (fun () -> Cons(Stop( d, (C,0)), list_to_stream_rec t)))
        | Note (p , d , v) -> fun () -> Cons(Tone (0.0 , p , v) , 
            (fun () -> Cons (Stop( d, p ), list_to_stream_rec t)))
  in list_to_stream_rec lst
;;
(* Stop duration, pitch
  Rest duration
  note pitch duration volume
tone duration pitch octave *)
...
let rec pair (a : event stream) (b : event stream) : event stream =
  (*interested in the time of head of both a and b to compare which came first*)
  let ha = head a in
  let hb = head b in
  (*when it's equal*)
  if time_of_event ha = time_of_event hb then
    fun () -> Cons ( ha , 
      fun () -> Cons( head (shift_start (-. time_of_event ha) b) , 
        (pair (tail a) (tail b) ) ))
  (* when hb happens at a later time *) 
  else if time_of_event ha < time_of_event hb then
    fun () -> Cons ( ha , pair (tail a) (shift_start (-. time_of_event ha) b) )
  (* when ha happens at a later time *)
  else 
    fun () -> Cons ( hb , pair (shift_start (-. time_of_event hb) a) (tail b))
;;
...
let transpose (str : event stream) (half_steps : int) : event stream =
    (* apply transpose to every elmt of the stream by writing function,
          then the targeted stream*)
    map ( fun x -> match x with
      (* if it's a tone, duration and volume stays same
          only pitch changes *)
      | Tone (d , p , v) -> Tone ( d , transpose_pitch p half_steps, v )
      (* if it's a stop, keep the duration but change the pitch *)
      | Stop (d , p) -> Stop ( d , transpose_pitch p half_steps)) str ;;
...
let canon = pair bass (pair (shift_start 2.0 melody) 
                (pair (shift_start 4.0 melody) (shift_start 6.0 melody)))
;;
