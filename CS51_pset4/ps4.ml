...
module BinSTree(C : COMPARABLE) : (BINTREE with type elt = C.t) =
  ...
  | hd :: _ -> 
    ...
    let getmin (t : tree) : elt = 
      let l, _ = pull_min t in List.hd (List.rev l)
    ...
    let rec getmax (t : tree) : elt = 
      match t with 
      | Leaf -> raise EmptyTree
      | Branch (_, lst, Leaf) -> List.hd (List.rev lst)
...
module ListQueueTesting = (ListQueue(IntCompare))
let _ = ListQueueTesting.run_tests()
...
module TreeQueue(C : COMPARABLE) : (PRIOQUEUE with type elt = C.t) =
  struct
    ...
    assert (take (add z (add y (add x empty))) = (z, add y (add x empty)))
  end

module IntTreeQ = TreeQueue(IntCompare)
let _ = IntTreeQ.run_tests ()
...
module BinaryHeap(C : COMPARABLE) : (PRIOQUEUE with type elt = C.t) =
  struct
    ...
    let rec fix (t : tree) : tree =
      ...
      | TwoBranch (bal, e1, t1, t2) ->
          match ( C.compare e1 (get_top t1), C.compare e1 (get_top t2)) with
          | (Greater, _) | (_, Greater) ->
            let swap (sm: tree) (gr: tree) : tree = 
              (match sm with
            | Leaf top -> 
              if sm = t1 then 
                 TwoBranch (bal, top, Leaf e1, gr)
            else TwoBranch (bal, top, gr, Leaf e1)
            | OneBranch (top, sm_tl) -> 
              if sm = t1 then 
                 TwoBranch (bal, top, fix (OneBranch (e1 ,sm_tl)), gr)
            else TwoBranch (bal, top, gr, fix (OneBranch (e1 ,sm_tl)))
            | TwoBranch (_, top, u1, u2) -> 
              if sm = t1 then 
                 TwoBranch (bal, top, gr, fix (TwoBranch (bal, e1, u1, u2)))
            else TwoBranch (bal, top, gr, fix (TwoBranch (bal, e1, u1, u2))))
            in
            if (get_top t1) < (get_top t2) then swap t1 t2
            else swap t2 t1
	       | (_, _) -> t
    ...
    let rec get_last (t : tree) : elt * queue =
      ...
      | TwoBranch(Even,h,t1,t2) -> 
        let (last,q) = get_last t2 in
          (match q with
          | Empty -> (last, Tree (OneBranch (h, get_top t1)))
          | Tree a -> (last, Tree (TwoBranch(Odd, h, t1, a)))) 
      | TwoBranch(Odd, h, t1, t2) -> 
        let (last,q) = get_last t1 in
          (match q with
          | Empty -> (last, Tree (OneBranch (h, get_top t2)))
          | Tree a -> (last, Tree (TwoBranch(Even, h, a, t2)))) 
    ...
    let take (q : queue) : elt * queue =
      ...
      | TwoBranch (Odd, e, t1, t2) -> 
          let (last, q1') = get_last t1 in
            match q1' with
            | Empty -> failwith "Impossible, min odd tree can't have leaf t1"
    ...
    let test_get_top () =
      ...
      let y = C.generate_gt x () in
      let z = C.generate_lt x () in
      assert (get_top( extract_tree (add z ( add y ( add x empty)))) = z)

    let test_get_last () = 
      let x3 = C.generate () in
      let x2 = C.generate_lt x3 () in
      let x4 = C.generate_gt x3 () in
      let x5 = C.generate_gt x4 () in
      assert (get_last (extract_tree (add x5 (add x2 (add x4 (add x3 empty)))))
           = (x5, Tree( TwoBranch (Even, x2, Leaf (x4), Leaf (x3))) ))
    ...
    let test_add () =
      ...
      assert ((add x3 empty) = (Tree (Leaf x3)));
      assert ((add x2 (add x3 empty)) = (Tree (OneBranch (x2, x3))));
      assert ((add x1 (add x2 (add x3 empty))) = 
             (Tree (TwoBranch (Even, x1, (Leaf x3), (Leaf x2)))));
      assert ((add x4 (Tree (TwoBranch (Even, x1, (Leaf x2), (Leaf x3))))) = 
             (Tree (TwoBranch (Odd, x1, OneBranch (x2, x4), (Leaf x3)))))
    ...
    let test_fix () =
      ...
      let x4 = C.generate_gt x3 () in
      let x5 = C.generate_gt x4 () in
      assert (fix (OneBranch (x4, x1)) = (OneBranch (x1, x4)));
      assert (fix (TwoBranch (Even, x3, OneBranch (x1,x4), OneBranch (x2,x5)))
           = (TwoBranch (Even, x1, OneBranch (x3,x4), OneBranch (x2,x5))))

    let test_take () =
      let x3 = C.generate () in
      let x2 = C.generate_lt x3 () in
      let x1 = C.generate_lt x2 () in
      assert (take (Tree (OneBranch (x1,x2))) = (x1, (Tree (Leaf x2))));
      assert (take (Tree (TwoBranch (Even, x1, (Leaf x2), (Leaf x3)))) 
           = (x1, (Tree (OneBranch (x2, x3)))))
  ...
  module HeapQueueTest = BinaryHeap(IntCompare)
  let _ = HeapQueueTest.run_tests ()
 ...
 * lists are returned in non-decreasing order! *)
 ...
 let test_heapsort () = 
    ...
    assert (heapsort [5;6;2;2;10] = [2;2;5;6;10])
;;

 let test_selectionsort () = 
    assert (selectionsort [5;4;3;2;1] = [1;2;3;4;5]);
    assert (selectionsort [5;6;2;2;10] = [2;2;5;6;10])
;;
