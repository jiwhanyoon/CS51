...
let gen_bear hive cave =
  ignore (new Bear.bear (0,0) hive cave)

let gen_cow hive past =
  ignore (new Cow.cow (World.size-1,World.size-1) hive past)
...
let part1_initializer () : unit =
  ...
  ignore (new Bear.bear (5,5) hive (hive :> world_object_i)) ;
  ignore (new Pasture.pasture (6,6) hive) ;
  ignore (new Cow.cow (7,7) hive (hive :> world_object_i));
  ignore hive

let part2_initializer () : unit =
  let hive = gen_hive () in
  ignore (new Bee.bee (World.size/2+1,World.size/2) (hive :> world_object_i)) ;
  gen_bear hive (hive :> world_object_i);
  gen_cow hive (hive :> world_object_i);
  ignore hive
