...
object (self)
  ...
  method private deposit_honey () =
    let rec make (i: int) : int list =
      if i > 0 then 1 :: (make (i - 1))
      else [] in
    if self#get_pos = home#get_pos then
    ignore (home#receive_pollen (make stolen_honey));
  ...
  method draw = self#draw_circle (Graphics.rgb 170 130 110) 
    Graphics.black ""
  ...
  (* ### TODO: Part 6 Custom Events ### *)
  method receive_sting = life <- life - 1
